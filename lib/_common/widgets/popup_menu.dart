  ...
  Widget _buildIconButton() {
    return Transform.rotate(
      ...
      child: SvgPicture.asset(
        ...
        height: 24.0,
        width: 24.0,
        color: ColorConstants.popupMenuIcon(widget.type),
      ),
    );
  }
        ...
        child: _DefaultPopupMenuButton<CustomPopupMenuItem>(
          ...
          itemBuilder: (_) {
            return widget.items.map((item) {
              return _DefaultPopupMenuItem<CustomPopupMenuItem>(
                  ...
                  child: CustomText(
                    ...
                    style: (widget.textStyle ?? Theme.of(context).textTheme.bodyMedium)?.copyWith(
                      color: item.isImportant
                          ? ColorConstants.popupMenuImportantText()
                          : ColorConstants.popupMenuText(),
                    ),
                  ),
                ),
              );
          ...
          shadowColor: ColorConstants.popupMenuShadow(),
          splashRadius: SizeConstants.defaultButtonIconSize,
          tooltip: '',
          clipBehavior: Clip.hardEdge,
          child: widget.icon ?? _buildIconButton(),
        ),
      ),
    );
  }
...
/// Custom PopupMenu implementation
const _kMenuDuration = Duration(milliseconds: 300);
const _kMenuCloseIntervalEnd = 2.0 / 3.0;
const _kMenuMaxWidth = 12.0 * _kMenuWidthStep;
const _kMenuMinWidth = 4.0 * _kMenuWidthStep;
const _kMenuWidthStep = 28.0;
const _kMenuScreenPadding = 8.0;
const _kMenuVerticalPadding = 0.0;

class _DefaultPopupMenuButton<T> extends StatefulWidget {
  ...
  @override
  _DefaultPopupMenuButtonState<T> createState() => _DefaultPopupMenuButtonState<T>();
}

class _DefaultPopupMenuButtonState<T> extends State<_DefaultPopupMenuButton<T>> {
  void showButtonMenu() {
    final popupMenuTheme = PopupMenuTheme.of(context);
    final button = context.findRenderObject()! as RenderBox;
    final overlay = Navigator.of(context).overlay!.context.findRenderObject()! as RenderBox;
    final popupMenuPosition = widget.position ?? popupMenuTheme.position ?? PopupMenuPosition.over;
    late Offset offset;
    ...
    switch (popupMenuPosition) {
      ...
      case PopupMenuPosition.under:
        ...
        if (widget.child == null) {
          offset -= Offset(0.0, widget.iconPadding.vertical / 2);
        }
    }
  ...
  bool get _canRequestFocus {
    final mode = MediaQuery.maybeNavigationModeOf(context) ?? NavigationMode.traditional;
    switch (mode) {
...
class _DefaultPopupMenuItem<T> extends PopupMenuEntry<T> {
  final T? value;
  ...
  @override
  _DefaultPopupMenuItemState<T, _DefaultPopupMenuItem<T>> createState() => _DefaultPopupMenuItemState<T, _DefaultPopupMenuItem<T>>();
}

class _DefaultPopupMenuItemState<T, W extends _DefaultPopupMenuItem<T>> extends State<W> {
  @protected
  ...
  Widget build(BuildContext context) {
    ...
    final popupMenuTheme = PopupMenuTheme.of(context);
    final defaults = _PopupMenuDefaultsM3(context);
    ...
    Widget item = AnimatedDefaultTextStyle(
      ...
      child: Container(
        ...
        padding: widget.padding ?? _PopupMenuDefaultsM3.menuHorizontalPadding,
        child: buildChild(),
      ),
    );
...
class _DefaultCheckedPopupMenuItem<T> extends _DefaultPopupMenuItem<T> {
  final bool checked;

  const _DefaultCheckedPopupMenuItem({
    super.key,
    super.value,
    this.checked = false,
    super.enabled,
  ...
  @override
  _DefaultPopupMenuItemState<T, _DefaultCheckedPopupMenuItem<T>> createState() => _DefaultCheckedPopupMenuItemState<T>();
}

class _DefaultCheckedPopupMenuItemState<T> extends _DefaultPopupMenuItemState<T, _DefaultCheckedPopupMenuItem<T>> with SingleTickerProviderStateMixin {
  ...
  Widget buildChild() {
    ...
    final effectiveLabelTextStyle = widget.labelTextStyle
...
class _PopupMenu<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < route.items.length; i += 1) {
      ...
      final end = clampDouble(start + 1.5 * unit, 0.0, 1.0);
    ...
    final opacity = CurveTween(curve: Interval(0.0, 1.0 / 3.0));
...
class _PopupMenuDefaultsM3 extends PopupMenuThemeData {
  ...
  @override
  ShapeBorder? get shape => RoundedRectangleBorder(borderRadius: BorderRadius.all(Radius.circular(4.0)));

  static EdgeInsets menuHorizontalPadding = EdgeInsets.symmetric(horizontal: 12.0);
}
